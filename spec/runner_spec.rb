...
RSpec.describe PostfixAdmin::Runner do
  ...
  describe "#add_account" do
    ...
    it "default scheme (CRAM-MD5) is applied" do
      ...
      expect(Mailbox.find('new-user@example.com').password).to eq CRAM_MD5_PASS
    end
    ...
    context "with scheme option" do
      %w[--scheme -s].each do |s_opt|
        ...
        # doveadm pw -u new-user@example.com -s DIGEST-MD5 -p password
        ...
        it "'#{s_opt}' allows to set DIGEST-MD5 schema" do
          ...
          expect(Mailbox.find(@user).password).to \
            eq "{DIGEST-MD5}364b46a317503b40f3430fd4eb946480"
        end
  ...
  describe "#setup" do
    ...
    it "adds a Domain and an Admin for it" do
      ...
      expect(Domain.exists?(@domain_name)).to be(true)
      expect(Admin.exists?(@admin)).to be(true)
      admin = Admin.find(@admin)
      expect(admin.rel_domains.exists?(@domain_name)).to be(true)
    end

    context "with description option" do
      it "adds a Domain with description" do
        res = capture do
          expect{
            Runner.start(@args + ["-d", "New Domain"])
          }.to change{ Admin.count }.by(1).and \
               change{ Domain.count }.by(1).and \
               change{ DomainAdmin.count }.by(1)
        end
  ...
  describe "#domains" do
    it "does not raise an error" do
      expect { silent { Runner.start(["domains"]) } }.not_to raise_error
    end
  end
