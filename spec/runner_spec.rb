...
describe PostfixAdmin::Runner do
  ...
  it "#setup" do
    capture(:stdout){ PostfixAdmin::Runner.start(['setup', 'example.net', 'password']) }.should =~ EX_REGISTERED
  ...
  describe "#add_alias and #delete_alias" do
    it "You can add and delete an new alias." do
      capture(:stdout){ PostfixAdmin::Runner.start(['add_alias', 'alias@example.com', 'goto@example.jp']) }.should =~ EX_REGISTERED
  ...
  it "#add_admin and #delete_admin" do
    capture(:stdout){ PostfixAdmin::Runner.start(['add_admin', 'admin@example.jp', 'password']) }.should =~ EX_REGISTERED
    capture(:stdout){ PostfixAdmin::Runner.start(['add_admin_domain', 'admin@example.jp', 'example.com']) }.should =~ EX_REGISTERED
  ...
  it "#add_account and #delete_account" do
    capture(:stdout){ PostfixAdmin::Runner.start(['add_account', 'user2@example.com', 'password']) }.should =~ EX_REGISTERED
